...
class TestYamlHandler(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.base_config = {
            "root_path": rf"U:\Users\Enlink\Documents\code\python\Xdm\IPC",  # str(Path(__file__).parent / "yaml_test" / "configs"),
            "file_pattern": ["*.yaml"],
        }
    ...
    def test_basic_yaml_tree(self):
        ...
        self.assertGreater(
            len(handler.file_tree.children), 0, "File tree should have children"
        )

        # Test data tree list
        self.assertGreater(
            len(handler.data_tree_list), 0, "Data tree list should not be empty"
        )
        ...
        if root_node:  # Type guard for root_node
            ...
            self.assertIn(
                "child1.yaml", child_names, "Root should have child1 as direct child"
            )
            self.assertIn(
                "child2.yaml", child_names, "Root should have child2 as direct child"
            )
        ...
        child1_data = next(
            (
                node
                for node in handler.data_tree_list
                if node._node.name == "child1.yaml"
            ),
            None,
        )
        ...
        if child1_data:  # Type guard for child1_data
            ...
            self.assertEqual(
                len(child1_data._node.children), 0, "Child1 should have no children"
            )
